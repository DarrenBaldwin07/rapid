...
describe('util', () => {
	...
	test('isDynamicRoute', () => {
		...
		expect(isDynamicRoute('/foo/_bar_/_baz_/_qux_/_quux_/_quuz_')).toEqual(
			true,
		);
	...
	test('generatePathUrl', () => {
		expect(
			generatePathUrl('/foo/_bar_', ['bar'], 'http://localhost:3000'),
		).toEqual('http://localhost:3000/foo/bar');
		expect(
			generatePathUrl(
				'/foo/_bar_/_baz_',
				['bar', 'baz'],
		...
		).toEqual('http://localhost:3000/foo/bar/baz');
		expect(
			generatePathUrl(
				'/foo/_bar_/_baz_/qux',
				['bar', 'baz'],
