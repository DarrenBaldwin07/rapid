...
import { RapidWebHandlerType, BoltRoutes, PostFunction, GetFunction, PatchFunction, PutFunction, DeleteFunction, Test } from './types';
 ...
 * @beta
 */
function createBoltClient<T extends RapidWebHandlerType, R extends BoltRoutes>(routes: BoltRoutes) {
    return <Key extends FetchKey<T> & string>(key: Key) => {
        ...
        switch (routeType) {
            case 'post':
                return {
                    post: <T = any, R = AxiosResponse<T, OutputMutationBody>, D = InputBody>(url: RequestUrl, data?: InputBody, config?: AxiosRequestConfig<D>): Promise<R> => axios.post(url, data, config),
                } as Test<T["mutations"][Key]["type"], RequestUrl, R, InputBody>;
            case 'get':
                return {
                    get: <T = any, R = AxiosResponse<T, OutputQueryBody>, D = any>(url: RequestUrl, config?: AxiosRequestConfig<D>): Promise<R> => axios.get(url, config),
                } as Test<T["mutations"][Key]["type"], RequestUrl, R, never>;
            case 'delete':
                return {
                    delete: <T = any, R = AxiosResponse<T, OutputMutationBody>, D = InputBody>(url: RequestUrl, config?: AxiosRequestConfig<D>): Promise<R> => axios.delete(url, config),
                } as Test<T["mutations"][Key]["type"], RequestUrl, R, InputBody>;
            case 'put':
                return {
                    put: <T = any, R = AxiosResponse<T, OutputMutationBody>, D = InputBody>(url: RequestUrl, data?: InputBody, config?: AxiosRequestConfig<D>): Promise<R> => axios.put(url, data, config),
                } as Test<T["mutations"][Key]["type"], RequestUrl, R, InputBody>;
            case 'patch':
                return {
                    patch: <T = any, R = AxiosResponse<T, OutputMutationBody>, D = InputBody>(url: RequestUrl, data?: InputBody, config?: AxiosRequestConfig<D>): Promise<R> => axios.patch(url, data, config),
                } as Test<T["mutations"][Key]["type"], RequestUrl, R, InputBody>;
        }
    };
}
