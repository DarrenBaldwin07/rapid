...
export const getDynamicSubstrings = (inputString: string): string[] => {
    ...
    const matches = inputString.match(regex); // returns an array of matches or null if no matches found
    if (matches === null) {
      return []; // return any empty array if no matches found (this should almost never happen)
    }
    return matches;
}
...
export const generatePathUrl = (path: string, updated_paths: Array<any>): string => {
    ...
    if (dynamicPaths.length > 0) {
        ...
        dynamicPaths.forEach((dynamicPath) => {
            const dynamicPathName = url.replace(dynamicPath, "");
        });
        return url;
    }
