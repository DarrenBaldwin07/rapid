...
const Switch = React.forwardRef<
	...
	SwitchProps
>(({ styles, ariaLabel, enabled, size, variant, enabledStyles, disabledStyles, ...rest }, ref) => {
  return (
      <HeadlessSwitch
        checked={enabled}
        ref={ref}
        ...
        className={RapidStyles(
            classes(enabled ? enabledStyles || 'bg-black' : disabledStyles || 'bg-lightGrey', 'inline-flex focus:ring-0 ring-0', enabled ? 'justify-end' : 'justify-start'),
      ...
      >
        ...
        <motion.span
          ...
          aria-hidden="true"
          className={classes('pointer-events-none inline-block h-[100%] aspect-square transform rounded-full bg-white shadow-lg ring-0 focus:ring-0')}
        />
      </HeadlessSwitch>
  )
});
