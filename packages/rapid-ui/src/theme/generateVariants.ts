...
function generateVariants(theme: VariantOutput, baseClassName: string) {
    const variants = Object.keys(theme.themeObject.variants || {}).filter(variant => isValidClassName(variant));
    const sizes = Object.keys(theme.themeObject.sizes || {}).filter(size => isValidClassName(size));
    const classNames = [];
